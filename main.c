...
Q1) We use both pre-test (while) and post-test (do-while) loops.
    A pre-test loop checks condition before entering the body.
    A post-test loop checks condition after one execution of the body.

Q2) The random number datatype generated by rand() is int.

Q3) The return type of play_user() is int because each round must produce
    a numeric round score that is added to the user's cumulative total.
*/
